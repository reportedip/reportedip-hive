...
class ReportedIP_Hive_Admin_Settings {
	...
	public static function render_mode_badge( $mode = null ) {
		...
		$community_reachable = ( ! is_multisite() || is_network_admin() )
			&& current_user_can( is_network_admin() ? 'manage_network_options' : 'manage_options' );
		$community_url       = $community_reachable
			? self::get_admin_page_url( 'admin.php?page=reportedip-hive-community' )
			: '';

		if ( '' !== $community_url ) {
			printf(
				'<a class="rip-mode-badge %1$s" href="%2$s" title="%3$s">',
				esc_attr( $info['badge_class'] ),
				esc_url( $community_url ),
				esc_attr__( 'Open the Community page', 'reportedip-hive' )
			);
		} else {
			printf( '<span class="rip-mode-badge %1$s">', esc_attr( $info['badge_class'] ) );
		}

		if ( 'local' === $info['key'] ) {
			echo '<svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>';
		} else {
			echo '<svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><circle cx="10" cy="10" r="8" stroke="currentColor" stroke-width="2" fill="none"/><path d="M2 10h16M10 2c2.8 2.8 4.4 6.5 4.4 8s-1.6 5.2-4.4 8c-2.8-2.8-4.4-6.5-4.4-8s1.6-5.2 4.4-8z" stroke="currentColor" stroke-width="1.5" fill="none"/></svg>';
		}

		echo esc_html( $info['label'] );

		echo '' !== $community_url ? '</a>' : '</span>';
	}
	...
	private function render_api_usage_card() {
		...
		$recent_total = (int) ( $stats_raw['recent_total'] ?? 0 );
		$lifetime     = (float) ( $stats_raw['success_rate'] ?? 0 );

		$has_window     = $recent_total >= 10;
		$success        = $has_window ? (float) ( $stats_raw['recent_success_rate'] ?? 100 ) : $lifetime;
		...
		$health_label   = $is_degraded
			...
			: esc_html__( 'Healthy', 'reportedip-hive' );
		...
		<div class="rip-card rip-mb-6">
			<div class="rip-card__header">
				...
				<div class="rip-header__actions">
					<?php if ( $has_window ) : ?>
						<span class="rip-badge rip-badge--<?php echo esc_attr( $health_variant ); ?>"><?php echo esc_html( $health_label ); ?></span>
					<?php endif; ?>
				...
				<div class="rip-stat-cards">
					...
					<div class="rip-stat-card">
						...
						<div class="rip-stat-card__content">
							...
							<div class="rip-stat-card__label"><?php echo esc_html( $rate_label ); ?></div>
						</div>
					</div>
					<div class="rip-stat-card">
						<div class="rip-stat-card__icon rip-stat-card__icon--info">
			...
			<div class="rip-card__footer">
				<button type="button" id="reset-api-stats" class="rip-button rip-button--secondary">
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>
					<?php esc_html_e( 'Reset API statistics', 'reportedip-hive' ); ?>
				</button>
			</div>
		</div>
		<?php
	...
	/**
	 * Render the "secured by ReportedIP Hive" note.
	 *
	 * Shared by the unified admin page footer and the profile 2FA section.
	 *
	 * @since 2.1.36
	 */
	public static function render_secured_by() {
		?>
	...
	/**
	 * Full menu for site admins on single-site and network admins on Multisite.
	 *
	...
	private function register_full_menu( $cap ) {
		...
		add_submenu_page( 'reportedip-hive', __( 'System Status', 'reportedip-hive' ), __( 'System Status', 'reportedip-hive' ), $cap, 'reportedip-hive-debug', array( $this, 'debug_page' ) );
	}
	...
	public function register_settings() {
		...
		register_setting(
			'reportedip_hive_hardening_master',
	 ...
	 */
	public function render_connection_card() {
		$mode_manager = ReportedIP_Hive_Mode_Manager::get_instance();
		?>
		<div class="rip-settings-section">
			<h2 class="rip-settings-section__title">
	...
	/**
	 * Tools › Data: the uninstall data switch (local option, Settings API).
	 *
	 * @return void
	 * @since  2.1.56
	 */
		...
		<div class="rip-card">
			...
			<div class="rip-card__body">
				<form method="post" action="<?php echo esc_url( self::settings_form_action() ); ?>">
					<?php settings_fields( 'reportedip_hive_uninstall' ); ?>
					...
					<?php submit_button( __( 'Save', 'reportedip-hive' ), 'rip-button rip-button--primary', 'submit', false ); ?>
				</form>
			</div>
		</div>
		<?php
	...
	/**
	 * Diagnostics tab of the Tools page: the branded test mail.
	 *
	 * @return void
	 * @since  2.1.56
	 */
	...
	private function get_plugin_health_status() {
		...
		try {
			...
			// phpcs:disable WordPress.DB.DirectDatabaseQuery -- One-time health probe on a plugin table; name from Schema::table() with a hardcoded suffix.
			...
			if ( ! $table_exists ) {
				...
				// phpcs:disable WordPress.DB.DirectDatabaseQuery,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- Safe table name composed from Schema::table() with a hardcoded suffix.
	 ...
	 */
	/**
	 * The three tabs of the Community page, slug to label.
	 *
	 * @return array<string,string>
	 * @since  2.1.57
	 */
	public static function community_tabs() {
		return array(
			'settings'  => __( 'Settings', 'reportedip-hive' ),
			'community' => __( 'Community', 'reportedip-hive' ),
			'badges'    => __( 'Badges', 'reportedip-hive' ),
		);
	}

	/**
	 * Resolve the active Community tab from the request.
	 *
	 * `tab` wins; the legacy `subtab=promote` of the former backlink tools
	 * lands on the badges tab; anything else opens the settings.
	 *
	 * @param mixed $tab    Raw `tab` query value.
	 * @param mixed $subtab Raw legacy `subtab` query value.
	 * @return string One of `settings`, `community`, `badges`.
	 * @since  2.1.57
	 */
	public static function community_tab( $tab, $subtab = '' ) {
		$tab = is_string( $tab ) ? sanitize_key( $tab ) : '';
		if ( isset( self::community_tabs()[ $tab ] ) ) {
			return $tab;
		}
		return 'promote' === ( is_string( $subtab ) ? sanitize_key( $subtab ) : '' ) ? 'badges' : 'settings';
	}
	...
	public function community_page() {
		...
		if ( isset( $_GET['rip_refresh'] ) && check_admin_referer( 'reportedip_hive_refresh_quota' ) ) {
			...
			wp_safe_redirect( self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=community&refreshed=1' ) );
			exit;
		}
		...
		$tab = self::community_tab( $_GET['tab'] ?? '', $_GET['subtab'] ?? '' );

		$shortcodes         = ReportedIP_Hive_Frontend_Shortcodes::get_instance();
		$contribution_stats = $shortcodes->get_cached_stats();
		$reports_30d        = (int) ( $contribution_stats['api_reports_30d'] ?? 0 );
		$reports_total      = (int) ( $contribution_stats['reports_total'] ?? 0 );
		$badge_active       = (bool) ReportedIP_Hive_Option_Routing::get( 'reportedip_hive_auto_footer_enabled', false );

		$contribution_args = array(
		...
		?>
			...
			<nav class="rip-nav-tabs" aria-label="<?php esc_attr_e( 'Community sections', 'reportedip-hive' ); ?>">
				<?php foreach ( self::community_tabs() as $slug => $label ) : ?>
					<a href="<?php echo esc_url( self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=' . $slug ) ); ?>" class="rip-nav-tabs__tab<?php echo $tab === $slug ? ' rip-nav-tabs__tab--active' : ''; ?>"><?php echo esc_html( $label ); ?></a>
				<?php endforeach; ?>
			</nav>
			...
			<div class="rip-content">
				...
				<?php endif; ?>
					...
					<div class="rip-alert rip-alert--info rip-mb-6">
						...
						<div class="rip-alert__content rip-alert__content--row">
							...
							<a href="<?php echo esc_url( self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=settings' ) ); ?>" class="rip-button rip-button--primary rip-button--sm">
					...
					<div class="rip-alert rip-alert--warning rip-mb-6">
						...
						<div class="rip-alert__content">
							...
							<div style="margin-top:12px; display:flex; gap:8px;">
								<a href="<?php echo esc_url( self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=settings' ) ); ?>" class="rip-button rip-button--primary">
				...
				<?php endif; ?>
					...
					<?php $this->render_community_hero( $contribution_args ); ?>

					<?php $this->render_contribution_panel( $contribution_args ); ?>
				...
				<?php else : ?>
					...
					<?php $this->render_backlink_booster(); ?>
	...
	/**
	 * Render the banner builder on the Badges tab.
	 *
	 * One preview, one shortcode: a row of templates sets variant, stat and
	 * tone in one click, the few visible fields adjust the look, and colours,
	 * text overrides and the attribute reference sit behind a disclosure.
	 * The one-click footer badge renders above it
	 * ({@see render_backlink_booster()}); the logic lives in
	 * `assets/js/badges.js`.
	 *
	 * @since 1.3.0
	 */
	private function render_promote_tab() {
		...
		$presets = array(
			array(
				'label'   => __( 'Footer badge', 'reportedip-hive' ),
				'variant' => 'badge',
				'type'    => 'attacks_total',
				'tone'    => 'protect',
			),
			array(
				'label'   => __( 'Shield icon', 'reportedip-hive' ),
				'variant' => 'shield',
				'type'    => 'attacks_total',
				'tone'    => 'protect',
			),
			array(
				'label'   => __( 'Threat counter', 'reportedip-hive' ),
				'variant' => 'stat',
				'type'    => 'attacks_total',
				'tone'    => 'trust',
			),
			array(
				'label'   => __( 'Community banner', 'reportedip-hive' ),
				'variant' => 'banner',
				'type'    => 'reports_total',
				'tone'    => 'community',
			),
			array(
				...
				'variant' => 'stat',
				'type'    => 'api_reports_30d',
				'tone'    => 'contributor',
			),
			array(
				'label'   => __( 'Login shield', 'reportedip-hive' ),
				'variant' => 'stat',
				'type'    => 'logins_30d',
				'tone'    => 'trust',
			),
		...
		$stat_types = array(
			'attacks_total'    => __( 'Attacks blocked, lifetime', 'reportedip-hive' ),
			'attacks_30d'      => __( 'Attacks blocked, last 30 days', 'reportedip-hive' ),
			'reports_total'    => __( 'Reports shared, lifetime', 'reportedip-hive' ),
			'api_reports_30d'  => __( 'Reports shared this month', 'reportedip-hive' ),
			'blocked_active'   => __( 'IPs currently blocked', 'reportedip-hive' ),
			'whitelist_active' => __( 'Trusted IPs', 'reportedip-hive' ),
			'logins_30d'       => __( 'Failed logins blocked, last 30 days', 'reportedip-hive' ),
			'spam_30d'         => __( 'Spam comments stopped, last 30 days', 'reportedip-hive' ),
		);

		$colours = array(
			'rip-cust-bg1'    => array( __( 'Background', 'reportedip-hive' ), '#4F46E5' ),
			'rip-cust-bg2'    => array( __( 'Gradient stop', 'reportedip-hive' ), '#7C3AED' ),
			'rip-cust-color'  => array( __( 'Foreground', 'reportedip-hive' ), '#ffffff' ),
			'rip-cust-border' => array( __( 'Border', 'reportedip-hive' ), '#ffffff' ),
		);
		...
		wp_localize_script(
			...
			array(
				...
				'strings'      => array(
					...
					'prompt' => __( 'Copy this shortcode:', 'reportedip-hive' ),
				),
			)
		);
		?>

		<div class="rip-card" id="rip-badge-builder">
			<div class="rip-card__header">
				<h2 class="rip-card__title">
			...
			<div class="rip-card__body">
				<p class="rip-badge-builder__intro">
					<?php esc_html_e( 'Pick a template, adjust a few options and copy the shortcode into any post, page, widget or template. The preview already shows the real numbers of this site.', 'reportedip-hive' ); ?>
				</p>

				<div class="rip-badge-presets" role="group" aria-label="<?php esc_attr_e( 'Templates', 'reportedip-hive' ); ?>">
					<?php foreach ( $presets as $preset ) : ?>
						<button type="button" class="rip-badge-preset" data-variant="<?php echo esc_attr( $preset['variant'] ); ?>" data-type="<?php echo esc_attr( $preset['type'] ); ?>" data-tone="<?php echo esc_attr( $preset['tone'] ); ?>"><?php echo esc_html( $preset['label'] ); ?></button>
					<?php endforeach; ?>
				</div>

				<div class="rip-badge-builder__grid">
					<div>
						<div class="rip-form-group">
							...
							<select id="rip-cust-variant" class="rip-input">
								<option value="badge" selected><?php esc_html_e( 'Badge, compact pill', 'reportedip-hive' ); ?></option>
								<option value="stat"><?php esc_html_e( 'Stat, single big number', 'reportedip-hive' ); ?></option>
								<option value="banner"><?php esc_html_e( 'Banner, full marketing block', 'reportedip-hive' ); ?></option>
								<option value="shield"><?php esc_html_e( 'Shield, icon only', 'reportedip-hive' ); ?></option>
							</select>
						</div>

						<div class="rip-form-group" id="rip-cust-type-group" hidden>
							<label class="rip-label" for="rip-cust-type"><?php esc_html_e( 'Number shown', 'reportedip-hive' ); ?></label>
							<select id="rip-cust-type" class="rip-input">
								<?php foreach ( $stat_types as $value => $label ) : ?>
						...
						<div class="rip-form-group">
							<label class="rip-label" for="rip-cust-tone"><?php esc_html_e( 'Wording', 'reportedip-hive' ); ?></label>
							<select id="rip-cust-tone" class="rip-input">
								<?php foreach ( ReportedIP_Hive_Frontend_Shortcodes::tone_definitions() as $tone => $definition ) : ?>
									<option value="<?php echo esc_attr( $tone ); ?>"><?php echo esc_html( $definition['headline'] ); ?></option>
								<?php endforeach; ?>
							</select>
						</div>

						<fieldset class="rip-form-group">
							<legend class="rip-label"><?php esc_html_e( 'Theme', 'reportedip-hive' ); ?></legend>
							<label class="rip-choice"><input type="radio" name="rip-cust-theme" value="dark" checked> <?php esc_html_e( 'Dark', 'reportedip-hive' ); ?></label>
							<label class="rip-choice"><input type="radio" name="rip-cust-theme" value="light"> <?php esc_html_e( 'Light', 'reportedip-hive' ); ?></label>
						</fieldset>

						<fieldset class="rip-form-group">
							<legend class="rip-label"><?php esc_html_e( 'Alignment', 'reportedip-hive' ); ?></legend>
							<label class="rip-choice"><input type="radio" name="rip-cust-align" value="left"> <?php esc_html_e( 'Left', 'reportedip-hive' ); ?></label>
							<label class="rip-choice"><input type="radio" name="rip-cust-align" value="center" checked> <?php esc_html_e( 'Center', 'reportedip-hive' ); ?></label>
							<label class="rip-choice"><input type="radio" name="rip-cust-align" value="right"> <?php esc_html_e( 'Right', 'reportedip-hive' ); ?></label>
						</fieldset>

						<details class="rip-badge-advanced">
							<summary><?php esc_html_e( 'Colours, texts and all attributes', 'reportedip-hive' ); ?></summary>

							<div class="rip-badge-colours">
								<?php foreach ( $colours as $id => $colour ) : ?>
									<div>
										<label class="rip-label rip-badge-colour__label" for="<?php echo esc_attr( $id ); ?>">
											<span><?php echo esc_html( $colour[0] ); ?></span>
											<button type="button" class="rip-badge-reset rip-cust-clear" data-target="<?php echo esc_attr( $id ); ?>"><?php esc_html_e( 'reset', 'reportedip-hive' ); ?></button>
										</label>
										<input type="color" id="<?php echo esc_attr( $id ); ?>" class="rip-input rip-badge-colour" value="<?php echo esc_attr( $colour[1] ); ?>" data-empty="1">
									</div>
								<?php endforeach; ?>
							</div>

							<div class="rip-form-group">
								<label class="rip-label" for="rip-cust-intro"><?php esc_html_e( 'Headline override', 'reportedip-hive' ); ?></label>
								<input type="text" id="rip-cust-intro" class="rip-input" maxlength="80" placeholder="<?php esc_attr_e( 'Leave empty to use the wording above', 'reportedip-hive' ); ?>">
							</div>

							<div class="rip-form-group">
								<label class="rip-label" for="rip-cust-label"><?php esc_html_e( 'Label override', 'reportedip-hive' ); ?></label>
								<input type="text" id="rip-cust-label" class="rip-input" maxlength="80" placeholder="<?php esc_attr_e( 'Leave empty to use the wording above', 'reportedip-hive' ); ?>">
							</div>

							<label class="rip-choice rip-form-group">
								<input type="checkbox" id="rip-cust-live" checked>
								<?php esc_html_e( 'Show the pulsing live dot', 'reportedip-hive' ); ?>
							</label>

							<div class="rip-badge-attributes">
								<p><strong><?php esc_html_e( 'Shortcodes', 'reportedip-hive' ); ?></strong>, <code>[reportedip_badge]</code>, <code>[reportedip_stat]</code>, <code>[reportedip_banner]</code>, <code>[reportedip_shield]</code></p>
					...
					<div>
						<div id="rip-cust-preview" class="rip-badge-preview rip-badge-preview--tall" data-align="center"></div>

						<label class="rip-label" for="rip-cust-shortcode"><?php esc_html_e( 'Your shortcode', 'reportedip-hive' ); ?></label>
						<div class="rip-badge-shortcode">
							<code id="rip-cust-shortcode">[reportedip_badge align="center"]</code>
						...
						<p class="rip-help-text">
							<?php esc_html_e( 'Paste it into any post, page, widget or template. The banner renders with exactly these settings and links back to reportedip.com.', 'reportedip-hive' ); ?>
						</p>
					</div>
				</div>
			</div>
	...
	/**
	 * Render the Community hero band.
	 *
	 * Light, design-system intro that frames participation as the page's
	 * purpose and surfaces a single headline contribution figure. The detailed
	 * per-channel status lives in {@see render_contribution_panel()}.
	 *
	 * @param array $args Shared community context (see community_page()).
	 * @return void
	 * @since  2.0.26
	 */
	private function render_community_hero( array $args ) {
		$reports_total = (int) ( $args['reports_total'] ?? 0 );
		?>
		<div class="rip-card rip-mb-6">
			<div class="rip-card__body">
				<h2 class="rip-card__title" style="margin-top:0;">
				...
				<?php if ( $reports_total > 0 ) : ?>
					<p style="margin-bottom:0;font-size:var(--rip-text-lg,1.125rem);">
						...
						echo wp_kses(
							sprintf(
								/* translators: %s: formatted number of threats this site has shared. */
								__( 'So far your site has shared <strong>%s threats</strong> with the community. Thank you.', 'reportedip-hive' ),
								esc_html( number_format_i18n( $reports_total ) )
							),
							array( 'strong' => array() )
						);
						?>
	...
	/**
	 * Render the one-click backlink (auto-footer badge) booster.
	 *
	 ...
	 * @since  2.0.26
	 */
	private function render_backlink_booster() {
		...
		<div class="rip-card rip-mb-6">
			...
			<div class="rip-card__body">
				...
				if ( isset( $_GET['settings-updated'] ) ) :
					?>
					<div class="rip-alert rip-alert--success rip-mb-4">
						<?php esc_html_e( 'Saved. Thank you for backing the hive.', 'reportedip-hive' ); ?>
					</div>
					<?php
				endif;
				?>
				...
				<?php if ( $auto_enabled ) : ?>
					<div class="rip-alert rip-alert--success rip-mb-4">
						<strong><?php esc_html_e( 'Your badge is live.', 'reportedip-hive' ); ?></strong>
						<?php esc_html_e( 'Your site now links back to the network and shows as a community member. You can change the look below.', 'reportedip-hive' ); ?>
					</div>
				<?php endif; ?>
				...
				<div id="rip-auto-footer-preview" class="rip-badge-preview" data-align="<?php echo esc_attr( $auto_align ); ?>">
					<?php
				...
				<form method="post" action="<?php echo esc_url( self::settings_form_action() ); ?>">
					...
					<input type="hidden" name="_wp_http_referer" value="<?php echo esc_attr( self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=badges' ) ); ?>">
					...
					<fieldset class="rip-form-group rip-mt-3">
						<legend class="rip-label"><?php esc_html_e( 'Variant', 'reportedip-hive' ); ?></legend>
						<label class="rip-choice">
					...
					<fieldset class="rip-form-group rip-mt-3">
						<legend class="rip-label"><?php esc_html_e( 'Position', 'reportedip-hive' ); ?></legend>
						<label class="rip-choice">
	...
	/**
	 * Render the "Your contribution & activity" panel.
	 *
	 * Three participation channels (share reports, footer badge, honeypot) shown
	 * as live status, followed by the 30-day activity figures and the
	 * share-your-reports recommendation for free, connected sites.
	 *
	 * @param array $args Shared community context (see community_page()).
	 * @return void
	 * @since  2.0.26
	 */
	private function render_contribution_panel( array $args ) {
		$reports_30d       = (int) ( $args['reports_30d'] ?? 0 );
		$badge_active      = ! empty( $args['badge_active'] );
		$is_honeypot       = ! empty( $args['is_honeypot'] );
		$is_community_mode = ! empty( $args['is_community_mode'] );
		$is_configured     = ! empty( $args['is_configured'] );
		$tier              = is_array( $args['tier'] ?? null ) ? $args['tier'] : array( 'slug' => 'free' );
		$upgrade_url       = (string) ( $args['upgrade_url'] ?? '' );
		$honeypot_url      = (string) ( $args['honeypot_url'] ?? '' );
		...
		$channels = array(
			array(
				'title'  => __( 'Share threats', 'reportedip-hive' ),
				'active' => $reports_active && $reports_30d > 0,
				'status' => $reports_active
					? ( $reports_30d > 0
			...
			array(
				...
				'action' => $badge_active ? null : array(
					'url'   => self::get_admin_page_url( 'admin.php?page=reportedip-hive-community&tab=badges' ),
			...
			array(
				...
				'action' => $is_honeypot ? null : array(
					'url'   => $honeypot_url,
					'label' => __( 'Learn more', 'reportedip-hive' ),
					'blank' => true,
				),
			),
		);
		...
		<div class="rip-card rip-mb-6">
			<div class="rip-card__header">
				<h2 class="rip-card__title">
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
					<?php esc_html_e( 'Your contribution & activity', 'reportedip-hive' ); ?>
				</h2>
			</div>
			<div class="rip-card__body">
				<div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:1em;margin-bottom:1.75em;">
					<?php foreach ( $channels as $channel ) : ?>
						<div class="rip-card" style="padding:1.25em;background:var(--rip-gray-50);">
							<div style="display:flex;align-items:center;justify-content:space-between;gap:.5em;margin-bottom:.5em;">
								<strong><?php echo esc_html( $channel['title'] ); ?></strong>
							...
							<p style="margin:0;color:var(--rip-gray-600);font-size:.9em;"><?php echo esc_html( $channel['status'] ); ?></p>
							<?php if ( ! empty( $channel['action'] ) && '' !== (string) $channel['action']['url'] ) : ?>
								<p style="margin:.75em 0 0;">
									<a href="<?php echo esc_url( $channel['action']['url'] ); ?>"<?php echo $channel['action']['blank'] ? ' target="_blank" rel="noopener"' : ''; ?> class="rip-button rip-button--secondary rip-button--sm"><?php echo esc_html( $channel['action']['label'] ); ?></a>
								</p>
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
				...
				<h3 style="margin:0 0 .75em;font-size:var(--rip-text-base,1rem);color:var(--rip-gray-700);"><?php esc_html_e( 'Last 30 days', 'reportedip-hive' ); ?></h3>
				...
				<?php if ( $total_events > 0 && 0 === $total_api_reports && 'free' === $tier['slug'] ) : ?>
					<div class="rip-alert rip-alert--info rip-mt-4">
	...
	public static function render_hardening_mode_tab() {
		...
		<div class="rip-settings-section">
			...
			<form method="post" action="<?php echo esc_url( self::settings_form_action() ); ?>" class="rip-form">
				<?php settings_fields( 'reportedip_hive_hardening_master' ); ?>
				...
				<p class="rip-help-text">
					<a href="<?php echo esc_url( self::get_admin_page_url( 'admin.php?page=reportedip-hive-protection#hardening_mode' ) ); ?>"><?php esc_html_e( 'The detection window, thresholds and duration live on the Protection page (Attack Response).', 'reportedip-hive' ); ?></a>
				</p>
