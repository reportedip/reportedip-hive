...
**What you can manage:** 63 settings across seven groups — detection thresholds and monitor toggles, auto-blocking and escalation, the WAF level and rule sync, hide-login, 2FA enablement/enforcement/policy, password policy, logging and retention, and notifications. `operation_mode`/`api_key`, the frontend-2FA slugs, hardening mode and the advanced security headers are deliberately excluded (connection- or side-effect-sensitive) and stay local. Coverage is identical on both dashboards because both render from the same site-exported schema; a new managed option appears in both after a schema reload.
